...
import 'brands.dart';
class RejectedBrand extends StatefulWidget {
  AddBrand? brandData;



  RejectedBrand({Key? key, this.brandData});

  @override
  State<RejectedBrand> createState() => _RejectedBrandState();
}

class _RejectedBrandState extends State<RejectedBrand> {
  TextEditingController search = TextEditingController();

  Stream<List<AddBrand>> getBrands() => FirebaseFirestore.instance
      .collection('brands')
      .where('delete', isEqualTo: false)
      .where('status', isEqualTo: 2).orderBy('rejectedDate',descending: true)
      .snapshots()
      .map((snapshot) =>
  ...
  Stream<List<AddBrand>> getSearchBrands() => FirebaseFirestore.instance
      .collection('brands')
      .where('delete', isEqualTo: false)
      .where('status', isEqualTo: 2).where('search',arrayContains: search.text.toUpperCase())
      .snapshots()
      .map((snapshot) =>
      snapshot.docs.map((doc) => AddBrand.fromJson(doc.data())).toList());
  int brandRef=0;
                              ...
                              return Expanded(
                                child: ListView.builder(
                                    ...
                                    itemBuilder:
                                      ...
                                      Timestamp atime =Timestamp.fromDate( brands[index].rejectedDate!);
