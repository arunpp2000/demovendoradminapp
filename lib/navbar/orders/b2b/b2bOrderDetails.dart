...
class B2bOrderDetailsPage extends StatefulWidget {

  final String id;

  var email;
  B2bOrderDetailsPage({Key? key,  required this.id,  }) : super(key: key);

  @override
  State<B2bOrderDetailsPage> createState() => _B2bOrderDetailsPageState();
}

class _B2bOrderDetailsPageState extends State<B2bOrderDetailsPage> {
  ...
  //   }
  // }
  getShipThrouth() async {
    List ordersItems = [];
    for(var a in Order!.items??[]){
      if(a['shipThrough']==true){
        ordersItems.add(a);
      }
      print('================');
      print(orderItems);
  ...
  shipRocket() async {
    ...
    if (token != null || token != '') {
      ...
      if (response.statusCode == 200) {
        ...
        await FirebaseFirestore.instance.collection('b2bOrders').doc(widget.id).update(data);
            ...
            builder: (buildContext) {
              return AlertDialog(
                ...
                actions: [
                  ...
                  TextButton(
                      onPressed: () async {
                        FirebaseFirestore.instance.collection('b2bOrders').doc(widget.id).update({
  ...
  bool  _selectOption1 = false;
  bool  _selectOption2 = false;
  bool  _selectOption3 = false;
  bool  _selectOption4 = false;
  // bool  _selectOption1 = false;
                    ...
                    onTap: () async {
                      ...
                      if (proceed) {
                        ...
                        if (Order!.orderStatus! > 2) {
                          ...
                          // );
                          await FirebaseFirestore.instance.collection('b2bOrders').doc(widget.id).update({
                            'orderStatus':2,
                            'returnOrder':true,
                            'invoiceNo':Order!.invoiceNo,
                            'cancelledDate':Timestamp.now(),
